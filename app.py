...
""" model1 = tf.keras.models.load_model(
    ".\model1_2EPOCHS_Xception_im_belin_reduit.h5", custom_objects={"get_f1": get_f1},
...
def extract(image):
    ...
    # print(image.split('/'))
   # cv2.imwrite(f"C:\Users\hassa\OneDrive\Bureau\flask\mask/{image.split('/')[1]}", segmented)
    cv2.imwrite(f".\\mask/{image.split('/')[1]}", segmented)
...
def predict():
    if "image" in request.files:
        ...
        image_file.save(".\\uploaded_image/" + image_file.filename)
        extraction = extract(".\\uploaded_image/" + image_file.filename)
        #prediction = predict_img(".\\uploaded_image/" + image_file.filename)
